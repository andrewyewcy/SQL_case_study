...
SELECT MAX( joindate ) joindate
FROM Members
) AS m2 ON m2.joindate = m1.joindate

/* Q7: Produce a list of all members who have used a tennis court. (XXX)
Include in your output the name of the court, and the name of the member
formatted as a single column. Ensure no duplicate data, and order by
the member name. */

SELECT m.surname, m.firstname, f.name AS facility_name
FROM Bookings AS b
...
SELECT bookid, surname, firstname, facility_name, starttime, slots, membercost, guestcost, booking_cost
FROM (
    SELECT b.bookid, m.surname, m.firstname, f.name AS facility_name, b.starttime, b.slots, f.membercost, f.guestcost,
        CASE WHEN m.memid =0
        THEN f.guestcost * b.slots
